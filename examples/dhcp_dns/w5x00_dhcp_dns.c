...
/* RTOS */
xSemaphoreHandle dns_sema = NULL;
...
int main()
{
    ...
    dns_sema = xSemaphoreCreateCounting((unsigned portBASE_TYPE)0x7fffffff, (unsigned portBASE_TYPE)0);
    vTaskStartScheduler();
...
void dns_task(void *argument)
{
    ...
    while(1)
    {
        xSemaphoreTake(dns_sema, portMAX_DELAY );
